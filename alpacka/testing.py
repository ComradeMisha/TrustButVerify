...
def run_with_dummy_network_prediction(coroutine, network_signature,
    ...
    Args:
        ...
        use_requests_wrapper (bool): True if network requests are using Request
            wrapper. False if they use legacy interface.
    ...
    try:
        ...
        while True:
            if use_requests_wrapper:
                assert request.type == RequestType.AGENT_PREDICTION
                batch_size = request.content.shape[0]
            else:
