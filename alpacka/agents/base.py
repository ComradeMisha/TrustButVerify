...
class OnlineAgent(Agent):
    ...
    def _handle_env_feedback(self, agent_info, action, next_observation, reward,
        ...
        """Handles feedback received from gym.Env.step() method.

        Called after every step on the environment being solved. Overriding
        is optional.

        It is intended for model-based agents, which use imperfect models.
        Agent may handle model's mispredictions here.

        Args:
            Same as received by the runner's callbacks.
            Additionally env_info returned by the env.step(...).
    ...
    def solve(self, env, epoch=None, init_state=None, time_limit=None):
        ...
        while not done:
            ...
            # It is no-op in the default implementation
            yield from self._handle_env_feedback(
                agent_info, action, next_observation, reward, done, info
            )
        ...
        solved = info['solved'] if 'solved' in info else False
        truncated = (info['TimeLimit.truncated']
                     if 'TimeLimit.truncated' in info else False)
        transition_batch = data.nested_stack(transitions)
        if self._use_trainable_env:
            trainable_env_info = yield from self._compute_trainable_env_info()
        else:
            trainable_env_info = None
        return data.Episode(
            transition_batch=transition_batch,
            return_=return_,
            solved=solved,
            truncated=truncated,
            trainable_env_info=trainable_env_info,
        )
